$(document).ready(function () {
  $(".first").on("click", function () {
    if ($("#email_phone").val() == "") {
      $(".checkEmailPhone").text("Email/Phone is Required");
    } else {
  ...
  $('#email_phone').on('input',function() {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        } else if (response == "exists_contact") {
          $("#email_phone").removeClass("is-invalid");
          $("#missing-feedback").html("");
          nextBtn.disabled = false;
        } else {
          $("#email_phone").addClass("is-invalid");
          $("#missing-feedback").html("Email/Phone not found.");
          nextBtn.disabled = true;
        }
      },
    });
...
function sendVerificationEmail() {
  ...
  if (!isNaN(email_phone)) {
    ...
    xhr.send("contact=" + email_phone);
  } else {
    // Call the sendEmail function via an AJAX request
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "./function/send_reset.php", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function () {
      if (this.readyState === XMLHttpRequest.DONE && this.status === 200) {
        // Request was successful, do something here if needed
