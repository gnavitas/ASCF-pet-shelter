...
<head>
   ...
   <title> <?= strtoupper($user_type) ?> | Transaction</title>
...
// Construct the SQL query with search term
...
if (!empty($search_term)) {
    $query .= " AND (id LIKE '%$search_term%'
                ...
                OR payment_reference_no LIKE '%$search_term%'
                OR payment_status LIKE '%$search_term%'
                OR payment_amount LIKE '%$search_term%'
                OR date_paid LIKE '%$search_term%')";
}
...
<H1>Transaction</h1>
    ...
    <input type="text" id="search" name="search" value="<?php echo $search_term; ?>" placeholder="Enter">
